...
public class Louvain implements Cloneable {
    ...
    // Map<账号id,标号>
    Map<String, Integer> nodeIdMap = new ConcurrentHashMap<String, Integer>();
    ...
    /**
     * <p>【描述】：处理数据，将账号id进行标号</p>
     * <p>【方法】：pretreatmentData</p>
     * <p>【参数】: [edgeList, userIds]</p>
     * <p>【作者】: lizheng</p>
     * <p>【日期】: 2019-08-13</p>
     **/
    public List<String> pretreatmentData(List<String> edgeList, List<String> userIds) {
        // 进行标号
        for (int i = 0; i < userIds.size(); i++) {
            String userId = userIds.get(i);
            nodeIdMap.put(userId, i);
        }
        // 转换为标号的边
        List<String> result = new ArrayList<String>();
        for (int i = 0; i < edgeList.size(); i++) {
//            System.out.println(i+"->"+edgeList.get(i));
            String[] split = edgeList.get(i).split(",");
            String node1 = split[0];
            String node2 = split[1];
            Integer node1No = nodeIdMap.get(node1);
            Integer node2No = nodeIdMap.get(node2);
            result.add(node1No + "," + node2No);
        }
     ...
     * <p>【日期】: 2019-08-13</p>
     **/
    public Map<String, Integer> changeData() {
        Map<Integer,Integer> clusterResult = new ConcurrentHashMap<Integer, Integer>();
        Map<String,Integer> result = new ConcurrentHashMap<String, Integer>();
        for (int i = 0; i < global_n; i++) {
//            System.out.println(i + ": " + global_cluster[i]);
            clusterResult.put(i,global_cluster[i]);
        }
        Set<String> keys = nodeIdMap.keySet();
        for (String key : keys) {
            Integer nodeNo = nodeIdMap.get(key);
            result.put(key,clusterResult.get(nodeNo));
        }
        return result;
    }
     ...
     * <p>【描述】：初始化数据</p>
     * <p>【方法】：init</p>
     * <p>【参数】: [edgeList, nodeNum, edgeNum]</p>
     * <p>【作者】: lizheng</p>
     * <p>【日期】: 2019-08-13</p>
     **/
    public void init(List<String> edgeList, int nodeNum, int edgeNum) {
